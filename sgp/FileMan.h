...
#define	MYGLOB_APPEND	0x0001	/* Append to output from previous call. */
#define	MYGLOB_DOOFFS	0x0002	/* Use gl_offs. */
#define	MYGLOB_ERR	0x0004	/* Return on error. */
#define	MYGLOB_MARK	0x0008	/* Append / to matching directories. */
#define	MYGLOB_NOCHECK	0x0010	/* Return pattern itself if nothing matches. */
#define	MYGLOB_NOSORT	0x0020	/* Don't sort. */
#define	MYGLOB_NOESCAPE	0x1000	/* Disable backslash escaping. */

#define	MYGLOB_NOSPACE	(-1)	/* Malloc call failed. */
#define	MYGLOB_ABORTED	(-2)	/* Unignored error. */
#define	MYGLOB_NOMATCH	(-3)	/* No match and MYGLOB_NOCHECK not set. */
#define	MYGLOB_NOSYS	(-4)	/* Function not supported. */

#define	MYGLOB_ALTDIRFUNC	0x0040	/* Use alternately specified directory funcs. */
#define	MYGLOB_BRACE	0x0080	/* Expand braces ala csh. */
#define	MYGLOB_MAGCHAR	0x0100	/* Pattern had my_globbing characters. */
#define	MYGLOB_NOMAGIC	0x0200	/* MYGLOB_NOCHECK without magic chars (csh). */
#define	MYGLOB_QUOTE	0x0400	/* Quote special chars with \. */
#define	MYGLOB_TILDE	0x0800	/* Expand tilde names from the passwd file. */
#define MYGLOB_LIMIT	0x2000	/* Limit pattern match output to ARG_MAX */
#define MYGLOB_ABEND	MYGLOB_ABORTED /* backward compatibility */
#define	MYGLOB_KEEPSTAT	0x4000	/* Retain stat data for paths in gl_statv. */
...
typedef struct {
	...
	int gl_flags;		/* Copy of flags parameter to my_glob. */
	char **gl_pathv;	/* List of paths matching pattern. */
	struct stat **gl_statv;	/* Stat entries corresponding to gl_pathv */
				/* Copy of errfunc parameter to my_glob. */
	int (*gl_errfunc)(const char *, int);

	/*
	 * Alternate filesystem access methods for my_glob; replacement
