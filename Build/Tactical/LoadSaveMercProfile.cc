...
void ExtractMercProfileUTF16(BYTE const* const Src, MERCPROFILESTRUCT& p)
{
	const BYTE* S = Src;
	...
	EXTR_U8(S, p.ubQuoteActionID) // XXX maybe should not get loaded, seems to contain garbage
	EXTR_U8(S, p.ubQuoteRecord) // XXX maybe should not get loaded, seems to contain garbage
	EXTR_U8(S, p.ubInvUndroppable)
	EXTR_U8A(S, p.ubRoomRangeStart, lengthof(p.ubRoomRangeStart)) // XXX maybe should not get loaded, seems to contain garbage
	EXTR_SKIP(S, 1)
	...
	EXTR_I32(S, p.iBalance) // XXX maybe should not get loaded, seems to contain garbage
	EXTR_SKIP(S, 2)
	...
	p.sPreCombatGridNo             = 0;
	p.ubTimeTillNextHatedComplaint = 0;
	p.ubSuspiciousDeath            = 0;
	p.iMercMercContractLength      = 0;
	p.uiTotalCostToDate            = 0;
...
void ExtractMercProfileFromFile(HWFILE const f, MERCPROFILESTRUCT& p)
{
#ifdef _WIN32 // XXX HACK000A
	BYTE Data[716];
#else
...
void InjectMercProfile(BYTE* const Dst, MERCPROFILESTRUCT const& p)
{
	...
	UINT32 const checksum = ProfileChecksum(p);
	INJ_U32(D, checksum)
